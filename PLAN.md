...
- **Phase 8 — SerilogTracing span support.** [SerilogTracing](https://github.com/serilog-tracing/serilog-tracing)
  bridges Serilog with `System.Diagnostics.Activity`: each completed span becomes an ordinary `LogEvent`
  carrying Serilog's *native* `Activity`-based `TraceId`/`SpanId` (CLEF's `@tr`/`@sp`, or the standard
  `JsonFormatter`'s top-level `TraceId`/`SpanId`, both siblings of `Properties` rather than inside it) plus
  `ParentSpanId`/`SpanKind`/`SpanStartTimestamp` tags — none of which the existing CLEF/`JsonFormatter`
  parsing captured (native trace fields were silently dropped; `@`-prefixed CLEF members were skipped
  outright). `SerilogEventParser` now reads both forms and promotes the resolved `TraceId`/`SpanId` into
  `Properties` too, so the pre-existing property-filter/correlation-key/export machinery (all of which only
  ever looked at `Properties`) picks them up for free. `StructuredLogEvent` gained `TraceId`/`SpanId` plus
  computed `ParentSpanId`/`SpanKind`/`SpanStartTimestamp`/`IsSpan`/`SpanDuration` (duration = `Timestamp -
  SpanStartTimestamp`, only when both `TraceId`+`SpanId`+`SpanStartTimestamp` are present — an ordinary log
  line emitted while a span is active carries the same trace/span ids via `Activity.Current` but never a
  start timestamp, which is how SerilogTracing itself recommends telling the two apart). A new pure
  `SerilogTraceTreeBuilder` (Core) groups a document's buffered span events by `TraceId` and nests them by
  `ParentSpanId` into a `TraceSpanNode` tree (orphaned parents — outside the buffered window — surface as
  their own roots rather than being dropped). On the WPF side: a "⏱ Xms" duration badge on span rows in the
  structured line template; a new non-modal "Trace Tree" panel (`TraceTreeViewModel`/`View`, 🕸 toolbar
  button + Ctrl+P palette entry + a per-line "View Trace Tree" context-menu item that preselects that
  line's trace) rendering the span hierarchy as a `TreeView`, double-click-to-navigate — modeled on the
  existing non-modal `DocumentStatsView` pattern (on-demand snapshot over the document's own buffer, not a
  live subscription). `samples/serilog-tracing/` (new, `generate.py` regenerates it deterministically) adds
  `orders-api.clef` — a fake SerilogTracing-instrumented service log (6 `GET /orders/{id}` requests with
  nested DB/HTTP child spans, 3 `GET /health` single-span traces, one Warning-level and one Error-level
  (with exception) request, and one span whose parent falls outside the file to exercise the orphan-root
  case) — for manually exercising the Trace Tree panel in the running app, per its README. No schema/settings
  changes. A second sample, `worker-batch.clef` (18 events, generated by the same script), covers a shape
  `orders-api.clef` doesn't: 3-level span nesting (`ProcessBatch` → `ProcessItem` → `SendEmail`) and two
  traces overlapping in wall-clock time, plus a partial-failure case where only the failing `ProcessItem`
  branch (and the root) escalates to `Error` while its sibling stays `Information`. Verified via 15 new
  Core tests (7 unit tests for the parser/tree-builder logic + 5 reading `orders-api.clef` + 3 reading
  `worker-batch.clef`, both end-to-end through the real `StructuredFileReader` → `SerilogTraceTreeBuilder`
  pipeline, so both sample files double as regression fixtures) + 3 new App tests (`TraceTreeViewModelTests`,
  exercised against a real `TailDocumentViewModel`/CLEF fixture, not mocks) — 248 Core / 111 App / 21 Mcp,
  all green — plus a full solution build and a non-interactive startup smoke test. **Still needs a manual
  interactive pass**: opening the Trace Tree panel from a running app against `orders-api.clef`, confirming
  the duration badge/tree render correctly and the pt-PT strings read well — an automated computer-use pass
